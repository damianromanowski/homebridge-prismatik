...
var EVENT_CONNECTION     = 0,
    ...
    EVENT_TURN_ON        = 10,
    EVENT_TURN_OFF       = 11,
    EVENT_SET_PROFILE    = 12,
    EVENT_SET_GAMMA      = 13,
    EVENT_SET_SMOOTH     = 14,
    EVENT_SET_BRIGHTNESS = 15,
    EVENT_SET_COLOR      = 16;
        ...
        if (isRunning && queue.length)
        {
            ...
            else if (callback)
            {
                switch(event)
                {
                    ...
                    case EVENT_LOCK:
                    case EVENT_UNLOCK:
                        callback.call(exports, (value == "success" || value == "not locked"));
                        break;
                    ...
                    case EVENT_GET_COLORS:
                        var colors = value.split(";");
                        callback.call(exports, colors);
                        break;

                    case EVENT_GET_STATUS:
                        callback.call(exports, value == "on")
                        break;
...
{
    queueEvent(EVENT_GET_COLORS, "getcolors", callback);
}
